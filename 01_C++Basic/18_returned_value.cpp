...
bool str_subrange(const string &str1, const string &str2)
{
    if (str1.size() == str2.size())
        return str1 == str2;
    auto size = (str1.size() < str2.size()) ? str1.size() : str2.size(); // 取较短的string长度
...
// 返回类类型的函数和调用运算符
string s1, s2;
auto sz = shorterString(s1, s2).size(); // 函数调用和点操作符优先级一样，服从左结合律
...
int odd[] = {1, 3, 5, 7, 9};
int even[] = {0, 2, 4, 6, 8};
decltype(odd) *arrPtr(int i) // decltype并不负责将数组类型转换成对应的指针，故需要加一个*
{
    return (i % 2) ? &odd : &even;
}
