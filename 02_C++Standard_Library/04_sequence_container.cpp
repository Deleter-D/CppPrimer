...
int main(int argc, char const *argv[]) {
  ...
  forward_list<string> words(
      articles.begin(), articles.end()); // 正确：将const char*元素转换为string
  auto it = authors.end();
  deque<string> authList(
      authors.begin(),
      it); // 两个迭代器表示一个范围，拷贝元素直到（但不包括）it指向的元素
  ...
  names.assign(oldstyle.cbegin(),
               oldstyle.cend()); // 正确：可以将const char*转换为string
...
/*
    ...
    deque        | 双端队列。支持快速随机访问。
                 | 在头尾位置插入或删除速度很快
    list         | 双向链表。只支持双向顺序访问。
                 | 在list中任何位置进行插入或删除速度都很快
    forward_list | 单向链表。只支持单向顺序访问。
                 | 在链表任何位置进行插入或删除操作速度都很快
    array        | 固定大小的数组。支持快速随机访问。不能添加或删除元素
    string       | 与vector相似的容器，但专用于保存字符。
                 | 随机访问快。在尾部插入或删除速度很快
    ———————————————————————————————————————————————————————————————
*/
...
/*
                                容器定义和初始化
    ——————————————————————————————————————————————————————————————————————
    C c;                | 默认构造函数。若C是一个array，
                        | 则c中元素按默认方式初始化，否则c为空
    ——————————————————————————————————————————————————————————————————————
    C c1(c2);           | c1初始化为c2的拷贝，c1和c2必须是相同类型的容器，
    C c1 = c2;          | 且保存的是相同的元素类型；
                        | 对于array，两者还必须大小相同
    ——————————————————————————————————————————————————————————————————————
    C c{a, b, c...};    | c初始化为初始化列表中元素的拷贝，
                        | 列表中元素的类型必须与c的元素类型相容
    C c = {a, b, c...}; | 对于array类型，列表中元素数目必须等于或小于
                        | array的大小，任何遗漏的元素都进行值初始化
    ——————————————————————————————————————————————————————————————————————
    C c(b, e);          | c初始化为迭代器b和e指定范围中的元素的拷贝，
                        | 范围中元素的类型必须与c的元素类型相容（array不适用）
    ——————————————————————————————————————————————————————————————————————
    只有顺序容器（不包括array）的构造函数才能接受大小参数
    C seq(n);           | seq包含n个元素，这些元素进行了值初始化，
                        | 该构造函数是explicit的（string不适用）
