...
void process_input(istream &in) { cin.good(); }
...
/*
    ...
    fstream fstrm;    | 创建一个未绑定的文件流，
                      | fstream是头文件fstream中定义的一个类
    fstream fstrm(s); | 创建一个fstream，并打开名为s的文件，
                      | 这些构造函数都是explicit的，
                      | 默认的文件模式mode依赖于fstream的类型 fstream
    fstrm(s, mode);   | 与前一个构造函数类似，但按指定mode打开文件
    fstrm.open(s);    | 打开名为s的文件，并将文件与fstrm绑定，
                      | 默认的文件模式mode依赖于fstream的类型，返回void
